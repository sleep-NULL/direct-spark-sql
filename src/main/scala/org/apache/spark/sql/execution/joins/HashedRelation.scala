...
private[joins] object UnsafeHashedRelation {
  ...
  val cache = CacheBuilder
    .newBuilder()
    .maximumSize(1000)
    .build[Seq[Expression], UnsafeProjection]()
  ...
  def apply(
    ...
    val keyGenerator = cache.get(key, new Callable[UnsafeProjection] {
      override def call(): UnsafeProjection = UnsafeProjection.create(key)
    })
  ...
  private def ensureAcquireMemory(size: Long): Unit = {
    ...
    if (got < size) {
      ...
      throw new SparkException(
        s"Can't acquire $size bytes memory to build hash relation, " +
          s"got $got bytes")
    }
  }
...
private[execution] case class HashedRelationBroadcastMode(key: Seq[Expression])
  ...
  override def transform(rows: Iterator[InternalRow], sizeHint: Option[Long]): HashedRelation = {
    sizeHint match {
      case Some(numRows) =>
