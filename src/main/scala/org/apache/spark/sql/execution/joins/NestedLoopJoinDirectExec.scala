...
import org.apache.spark.sql.execution.direct.{BinaryDirectExecNode, DirectPlan}
import org.apache.spark.sql.execution.metric.SQLMetrics
import org.apache.spark.sql.internal.SQLConf
import org.apache.spark.util.collection.{BitSet, CompactBuffer}

case class NestedLoopJoinDirectExec(
                               left: DirectPlan,
                               right: DirectPlan,
                               buildSide: BuildSide,
                               joinType: JoinType,
                               condition: Option[Expression]) extends BinaryDirectExecNode {
   ...
   */
  private def innerJoin(relation: Array[InternalRow]): Iterator[InternalRow] = {
    val streamedIter = streamed.execute()
    val buildRows = relation
   ...
   */
  private def outerJoin(relation: Array[InternalRow]): Iterator[InternalRow] = {
    val streamedIter = streamed.execute()
      val buildRows = relation
      val joinedRow = new JoinedRow
  ...
  private def leftExistenceJoin(
                                 relation: Array[InternalRow],
                                 exists: Boolean): Iterator[InternalRow] = {
  ...
  private def existenceJoin(relation: Array[InternalRow]): Iterator[InternalRow] = {
    assert(buildSide == BuildRight)
    val streamedIter = streamed.execute()
      val buildRows = relation
      val joinedRow = new JoinedRow
   ...
   */
  private def defaultJoin(relation: Array[InternalRow]): Iterator[InternalRow] = {
    /** All rows that either match both-way, or rows from streamed joined with nulls. */
    val streamedIter = streamed.execute().toArray

    val matchedBuildRows = {
      val buildRows = relation
    ...
    (matchedStreamRows ++ notMatchedBroadcastRows).iterator
  }

  protected override def doExecute(): Iterator[InternalRow] = {
    val broadcastedRelation = broadcast.execute().toArray
